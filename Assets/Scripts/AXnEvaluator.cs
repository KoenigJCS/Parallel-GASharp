...
{
    public AXnEvaluator(int seed = 0)
    {
    ...
    public static float Decode(int start, int end, float min, float precision, int[] chromosome)
    {
        float sum = 0;
        for(int i = start; i < end; i++) {
            sum += chromosome[i] * Mathf.Pow(2, i - start);
        }
        return min + (precision * sum);
    }
    ...
    public float F4Chromosome(int[] chromosome)
    {
        float sum = 0;
        float[] x = new float[30];
        for(int i = 0; i < 30; i++) {
            x[i] = Decode(i * 8, i * 8 + 8, -1.28f, precision, chromosome);
            sum += i * Mathf.Pow(x[i], 4f) ;
            //InputHandler.inst.ThreadLog("F3: x: " + x + ", " + x);
        }
        sum += (float) GenerateGaussianNoise(0, 1);
